
#ifndef options_LUGE7_SN179_H
#define	options_LUGE7_SN179_H

#define CUSTOM_OFFSETS
#define XACCEL_OFFSET	( -22 )
#define YACCEL_OFFSET	( -24 )
#define ZACCEL_OFFSET	( 79 )
#define GYRO_OFFSET_TABLE "../libUDB/gyro_tables/table_luge7_sn179.h"
#define CALIBRATIONX	1.0134
#define CALIBRATIONY	1.0134
#define CALIBRATIONZ	1.0134
#define CALIB_GRAVITY	4096
#define CAL_GRAV_X	4100
#define CAL_GRAV_Y	4106
#define CAL_GRAV_Z	4094

#define X_CROSS_COUPLING 0
#define Y_CROSS_COUPLING 0
#define Z_CROSS_COUPLING 0

#endif	/* options_LUGE7_SN179_H */

