
#ifndef options_LUGE7_SN132_H
#define	options_LUGE7_SN132_H

#define CUSTOM_OFFSETS
#define XACCEL_OFFSET	( 14 )
#define YACCEL_OFFSET	( -24 )
#define ZACCEL_OFFSET	( 3 )
#define GYRO_OFFSET_TABLE "../libUDB/gyro_tables/table_luge7_sn132.h"
#define CALIBRATIONX	1.0134
#define CALIBRATIONY	1.0134
#define CALIBRATIONZ	1.0134
#define CALIB_GRAVITY	4096
#define CAL_GRAV_X	4105
#define CAL_GRAV_Y	4098
#define CAL_GRAV_Z	4034

#define X_CROSS_COUPLING 32
#define Y_CROSS_COUPLING 0
#define Z_CROSS_COUPLING 0

#endif	/* options_LUGE7_SN132_H */

