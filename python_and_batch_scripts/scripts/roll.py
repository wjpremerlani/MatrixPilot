...
debug_file = open("roll_log_file.txt" , "w" )
debug_file.write(f"roll log file opened.\n")
...
if plotlet_file is not None:
    ...
    with rc :
        ...
        try :
            roll_df = plotlets_df[roll_columns]
            roll_chart = st.line_chart(roll_df,y_label = "roll, degrees")
        except :
            debug_file.write(f"roll chart failed.\n")

    with rrc :

        try :
            roll_rate_df = plotlets_df[roll_rate_columns]
            roll_rate_chart = st.line_chart(roll_rate_df,y_label = "roll rate, deg/sec")
        except :
            debug_file.write(f"roll rate chart failed.\n")
    ...
    if curve_number is not None:
        ...
        curvelet_roll = curvelet_df[roll_columns]
        with rc :
            curvelet_roll_chart = st.line_chart(curvelet_roll,y_label = "roll, degrees")
        curvelet_roll_rate = curvelet_df[roll_rate_columns]
        with rrc :
            curvelet_roll_rate_chart = st.line_chart(curvelet_roll_rate,y_label = "roll rate, deg/sec")
