...
debug_file = open("yaw_log_file.txt" , "w" )
debug_file.write(f"yaw log file opened.\n")
...
if plotlet_file is not None:
    ...
    yaw_column , yaw_rate_column = st.columns(2)

    with yaw_column :

        try :
            yaw_df = plotlets_df[yaw_columns]
            yaw_chart = st.line_chart(yaw_df,y_label = "yaw, degrees")
        except :
            debug_file.write(f"yaw chart failed.\n")

    with yaw_rate_column :

        try :
            yaw_rate_df = plotlets_df[yaw_rate_columns]
            yaw_rate_chart = st.line_chart(yaw_rate_df,y_label = "yaw rate, deg/sec")
        except :
            debug_file.write(f"yaw rate chart failed.\n")
    ...
    if curve_number is not None:
        ...
        curvelet_yaw = curvelet_df[yaw_columns]
        with yaw_column :
            curvelet_yaw_chart = st.line_chart(curvelet_yaw,y_label = "yaw, degrees")
        curvelet_yaw_rate = curvelet_df[yaw_rate_columns]
        with yaw_rate_column :
            curvelet_yaw_rate_chart = st.line_chart(curvelet_yaw_rate,y_label = "yaw rate, deg/sec")
