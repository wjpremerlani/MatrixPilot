...
debug_file = open("force_log_file.txt" , "w" )
debug_file.write(f"force log file opened.\n")
...
if plotlet_file is not None:
    ...
    with fz :
        ...
        try :
            z_force_df = plotlets_df[z_force_columns]
            z_force_chart = st.line_chart(z_force_df,y_label = "z force, ft/sec/sec")
        except :
            debug_file.write(f"z force chart failed.\n")

    with fy :

        try :
            y_force_df = plotlets_df[y_force_columns]
            y_force_chart = st.line_chart(y_force_df,y_label = "y force, ft/sec/sec")
        except :
            debug_file.write(f"y force chart failed.\n")
    ...
    if curve_number is not None:
        ...
        curvelet_z_force = curvelet_df[z_force_columns]
        with fz :
            curvelet_z_force_chart = st.line_chart(curvelet_z_force,y_label = "z force, ft/sec/sec")
        curvelet_y_force = curvelet_df[y_force_columns]
        with fy :
            curvelet_y_force_chart = st.line_chart(curvelet_y_force,y_label = "y force, ft/sec/sec")
